...
int main(int argc, char** argv) {
  ...
  desc.add_options()
    ...
    ("core-r0", po::value<float>(&core_r0)->default_value(0.1), "Start learning rate for core features")
    ...
    ("boost", po::value<float>(&boost)->default_value(1.0), "Apply boosting factor to updates on misranked candidates")
  ...
  if (kbest) {
    ...
    realBleu = false;
    disableBleuFeature = false;
    cerr << "Use hope-fear decoding objective.." << endl;
  }
...
//	int sumStillViolatedConstraints;
  ...
  for (size_t epoch = continue_epoch; epoch < epochs && !stop; ++epoch) {
	...
	if (feature_confidence) {
	  cerr << "Rank " << rank << ", epoch " << epoch << ", apply feature learning rates with decays core=" << decay_core << "/sparse=" << decay_sparse << ", current learning rates: " << featureLearningRates << endl;
	  if (model_hope_fear) {
